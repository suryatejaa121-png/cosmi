...
function footerModalRegion(html, name) {
  const a = html.indexOf('<div class="modals-wrapper _1 footer">');
  const b = html.indexOf('<div class="modal-search">');
  if (a < 0 || b < a) throw new Error(`${name}: footer modals region not found`);
  return [a, b];
}
function cosmironFooterModals(html, R) {
  const home = read(HOME_PAGE);
  const [ha, hb] = footerModalRegion(home, HOME_PAGE);
  const block = home.slice(ha, hb).split('"./').join(`"${R}`).split(", ./").join(`, ${R}`);
  const [a, b] = footerModalRegion(html, SHELL_PAGE);
...
function shell({ file, title, description, body, jsonLd, current = "work-link", css = [], js = [] }) {
  ...
  // The About shell still carries the template's EternaCloud footer popups (Security / Privacy / Terms);
  // use the homepage's Cosmiron ones instead.
  html = cosmironFooterModals(html, R);
